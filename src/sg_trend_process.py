...
log = logging.getLogger("sg_trend_process")
...
@dataclass
class CFG:
    BASE_DIR: Path = Path(config.DATA_DIR)
    ...
    SG_CALENDAR_PATH: Path = field(init=False)
    SG_TARGET_PATH:   Path = field(init=False)
    XLARGE_PATH: Path = field(init=False)
    ...
    FORCE_REBUILD_CARRY: bool = False
    ...
    CARRY_APPLY_COMMODITY_SIGN: bool = True
    ...
    def __post_init__(self):
        ...
        self.SG_CALENDAR_PATH = Path(config.SG_TREND_INPUT_DIR) / "sg_trend_calendar.pkl"
        self.SG_TARGET_PATH   = Path(config.SG_TREND_INPUT_DIR) / "y_excess_NEIXCTAT.pkl"
        self.XLARGE_PATH = Path(config.PANEL_OUTPUT_DIR) / "X_large_universe" / "X_large_universe.parquet"
        base_viz = Path(config.FIGURES_DIR) / "sg_trend_visualizations"
        self.VIZ_DIR = base_viz / "process_based_results" / "SG Trend"
        self.VIZ_DIR.mkdir(parents=True, exist_ok=True)
        self.OUT_DIR = self.VIZ_DIR
...
print(f"Final volatility target: {CFG.FINAL_VOL_TARGET:.0%}")
...
def ann(x: pd.Series) -> float:
    return float(x.std() * np.sqrt(CFG.ANNDAYS if hasattr(CFG, "ANNDAYS") else CFG.ANN_DAYS))
...
def rescale_series_to_target(ret: pd.Series, target: float,
                             span: int = CFG.EWMA_SPAN_VOL,
                             cap: float = np.inf, floor: float = 0.0,
                             keep_na: bool = True) -> pd.Series:
...
def renorm_with_cap(beta: pd.Series, cap: float, l1_target: float) -> pd.Series:
    ...
    for _ in range(200):
        ...
        if residual <= 1e-12:
            break
        under_mask = w < cap - 1e-15
        if not under_mask.any():
            w *= (l1_target / float(w.sum()))
            break
        base = beta.clip(lower=0.0).astype(float).loc[under_mask]
        if base.sum() <= 1e-15:
            add = residual / int(under_mask.sum())
            w.loc[under_mask] += add
        else:
            w.loc[under_mask] += residual * (base / float(base.sum()))
...
def availability_adjust_weights(W: pd.DataFrame,
                                R: pd.DataFrame,
                                cap: float = CFG.MAX_SINGLE_WEIGHT,
                                l1_target: float = CFG.L1_TARGET) -> pd.DataFrame:
...
SIGN_MAP = {
    ...
    "GAS-LAST": -1, "GAS-PEN": -1, "GAS_US": -1,
    "GASOIL": -1, "GASOILINE": -1, "GASOILINE_ICE": -1,
    "HEATOIL": -1, "HEATOIL-ICE": -1,
    "NATGAS": -1, "EUA": -1,
    "COAL": -1, "IRON": -1, "STEEL": -1,  "GAS_US_mini": -1,
    "ALUMINIUM": -1, "ALUMINIUM_LME": -1,
    "GOLD": -1, "GOLD-mini": -1, "SILVER": -1,
...
_COMMODITY_STEMS = {
    "BRENT","WTI","CRUDE","OIL","GASOIL","HEATOIL","RBOB","GASOLINE","NATGAS",
    "GOLD","SILVER","PLATIN","PALLAD","ALUMIN","ALUMINIUM","COPPER","ZINC","TIN","LEAD","NICKEL",
    "COAL","IRON","STEEL","COFFEE","ROBUSTA","COCOA","COTTON","SUGAR","CORN","WHEAT","SOY","OATS","RICE",
    "CATTLE","HOG","BUTTER","CHEESE","MILK","WHEY","RUBBER","ETHANOL","EUA"
}

def _carry_tokenize(sym: str) -> list[str]:
    return [t for t in re.split(r'[^A-Z0-9]+', str(sym).upper()) if t]

def _carry_looks_like_commodity_symbol(sym: str) -> bool:
    s = str(sym).upper()
    if s in NON_COMMODITY_OVERRIDES:
        return False
    parts = _carry_tokenize(s)
...
def _carry_cap_for_symbol(sym: Optional[str]) -> float:
    s = (sym or "").upper()
    if s in NON_COMMODITY_OVERRIDES:
        return CFG.CARRY_K_MAX_EQ
    if s in SIGN_MAP and SIGN_MAP.get(s, +1) == -1:
        return CFG.CARRY_K_MAX
    return CFG.CARRY_K_MAX if _carry_looks_like_commodity_symbol(s) else CFG.CARRY_K_MAX_EQ

def _carry_read_csv_fast(path: Path, **kwargs) -> pd.DataFrame:
    try:
        return pd.read_csv(path, engine="pyarrow", **kwargs)
    except Exception:
        return pd.read_csv(path, **kwargs)

def _carry_parse_dt_mixed(x) -> pd.DatetimeIndex:
...
def _carry_to_four_digit_year(y: str) -> str:
    if len(y) == 4 and y.isdigit(): return y
    if len(y) == 2 and y.isdigit():
        yy = int(y); return f"19{y}" if yy >= 70 else f"20{y}"
    return ""

def _carry_canon_yyyymm(x: object):
    s = str(x).strip().upper()
    ...
    if m:
        ...
        if g1.isalpha(): mon = _MONTH_LETTER.get(g1, ""); year = _carry_to_four_digit_year(g2)
        else:            year = _carry_to_four_digit_year(g1); mon = _MONTH_LETTER.get(g2, "")
        ym = (year + mon) if (year and mon) else ""
        return ym if len(ym) == 6 else np.nan
    m = re.search(r'(' + '|'.join(_MONTH_NAME.keys()) + r')\s*[-/]?\s*(\d{2,4})', s) or \
        re.search(r'(\d{2,4})\s*[-/]?\s*(' + '|'.join(_MONTH_NAME.keys()) + r')', s)
    if m:
        if m.group(1) in _MONTH_NAME:
            mon = _MONTH_NAME[m.group(1)]; year = _carry_to_four_digit_year(m.group(2))
        else:
            year = _carry_to_four_digit_year(m.group(1)); mon = _MONTH_NAME[m.group(2)]
        ym = (year + mon) if (year and mon) else ""
        return ym if len(ym) == 6 else np.nan
...
def _carry_looks_like_roll_header(cols: List[str]) -> bool:
    ...
    looks_price = (("price" in L) and ("forward" in L)) or \
                  (any("price_contract" in c for c in L) and any("forward_contract" in c for c in L))
    return has_date and has_curr and has_carry and not looks_price

def _carry_looks_like_price_header(cols: List[str]) -> bool:
    L = [c.lower() for c in cols]
    req = {"datetime","price","price_contract","forward","forward_contract"}
    return req.issubset(set(L))

def _carry_classify_csv(path: Path) -> str:
    try:
        head = _carry_read_csv_fast(path, nrows=32); cols = list(head.columns)
    except Exception:
        return "unknown"
    if _carry_looks_like_roll_header(cols):  return "roll"
    if _carry_looks_like_price_header(cols): return "price"
    return "unknown"

def _carry_discover_rolls(rolls_dir: Path) -> List[Path]:
    files = [p for p in rolls_dir.glob("*.csv") if p.is_file()]
    rolls = [p for p in files if _carry_classify_csv(p) == "roll"]
    if not rolls:
        log.error(f"No roll calendars (by header) found in {rolls_dir}")
    else:
        log.info(f"Found {len(rolls)} roll calendars by header in {rolls_dir}")
    return sorted(rolls)

def _carry_price_path_for_symbol(symbol: str, prices_dir: Path) -> Optional[Path]:
    p = prices_dir / f"{symbol}.csv"
    if p.exists() and _carry_classify_csv(p) == "price":
        return p
    log.warning(f"{symbol}: no exact price file found in {prices_dir}")
    return None

def load_roll_calendar(csv_path: Path) -> pd.DataFrame:
    df = _carry_read_csv_fast(csv_path)
    ...
    r["DATE_TIME"]        = _carry_parse_dt_mixed(r["DATE_TIME"])
    r["current_contract"] = r["current_contract"].map(_carry_canon_yyyymm)
    r["carry_contract"]   = r["carry_contract"].map(_carry_canon_yyyymm)
...
def load_price_data(csv_path: Path) -> pd.DataFrame:
    ...
    d["DATETIME"]         = _carry_parse_dt_mixed(d["DATETIME"])
    d["PRICE_CONTRACT"]   = d["PRICE_CONTRACT"].map(_carry_canon_yyyymm)
    d["FORWARD_CONTRACT"] = d["FORWARD_CONTRACT"].map(_carry_canon_yyyymm)
    d = (d.dropna(subset=["DATETIME","PRICE","FORWARD","PRICE_CONTRACT","FORWARD_CONTRACT"])
          .drop_duplicates(subset=["DATETIME","PRICE_CONTRACT","FORWARD_CONTRACT"])
          .sort_values("DATETIME"))
    return d

def _carry_flip_if_needed(base: pd.DataFrame) -> Tuple[pd.DataFrame, bool]:
    ...
    if flip_score > thresh:
        base = base.copy()
        base.rename(columns={
            "PRICE": "TMP_P", "PRICE_CONTRACT": "TMP_PC",
            "FORWARD": "PRICE", "FORWARD_CONTRACT": "PRICE_CONTRACT"
        }, inplace=True)
        base.rename(columns={"TMP_P": "FORWARD", "TMP_PC": "FORWARD_CONTRACT"}, inplace=True)
        flipped = True
    return base, flipped

def _carry_ym_int(s: pd.Series) -> pd.Series:
    y = pd.to_numeric(s.str.slice(0, 4), errors="coerce")
    m = pd.to_numeric(s.str.slice(4, 6), errors="coerce")
    return (y * 12 + m).astype("Int64")

def _carry_maybe_flip_sign(sym: str, series: pd.Series) -> pd.Series:
    sym_norm = str(sym).upper()
    if sym_norm in SIGN_MAP:
        return series * SIGN_MAP[sym_norm]
    if sym_norm in NON_COMMODITY_OVERRIDES:
        return series
    if CFG.CARRY_APPLY_COMMODITY_SIGN and _carry_looks_like_commodity_symbol(sym_norm):
        return -series
    return series

def compute_raw_forecast(roll_daily: pd.DataFrame, price_df: pd.DataFrame,
                         symbol: Optional[str] = None) -> Tuple[pd.Series, Dict[str, int]]:
    base = (price_df.merge(roll_daily, left_on="DATETIME", right_index=True, how="inner")
                    .loc[:, ["DATETIME","PRICE","FORWARD",
                             "PRICE_CONTRACT","FORWARD_CONTRACT",
                             "current_contract","carry_contract"]]
                    .dropna(subset=["PRICE_CONTRACT","FORWARD_CONTRACT","current_contract","carry_contract"])
                    .copy())
    base, flipped = _carry_flip_if_needed(base)
    ...
    def _pick_best_per_date(df: pd.DataFrame, same_front_only: bool) -> pd.DataFrame:
        ...
        if g.empty: return g
        pc_ym  = _carry_ym_int(g["PRICE_CONTRACT"]).astype("float64")
        fc_ym  = _carry_ym_int(g["FORWARD_CONTRACT"]).astype("float64")
        cur_ym = _carry_ym_int(g["current_contract"]).astype("float64")
        car_ym = _carry_ym_int(g["carry_contract"]).astype("float64")
    ...
    if not same_front_best.empty:
        pc = _carry_ym_int(same_front_best["PRICE_CONTRACT"]).astype("float64")
        fc = _carry_ym_int(same_front_best["FORWARD_CONTRACT"]).astype("float64")
        far_contract = np.where(pc <= fc,
                                same_front_best["FORWARD_CONTRACT"],
                                same_front_best["PRICE_CONTRACT"])
    ...
    if matched.empty:
        try:
            if symbol:
                cols = ["DATETIME","PRICE_CONTRACT","FORWARD_CONTRACT","current_contract","carry_contract","PRICE","FORWARD"]
                (CFG.CARRY_SIGNALS_DIR / "_debug").mkdir(exist_ok=True)
                base.loc[:, cols].tail(300).to_csv(CFG.CARRY_SIGNALS_DIR / "_debug" / f"{symbol}_debug_pairs.csv", index=False)
        except Exception:
            pass
    ...
    pc_ym = _carry_ym_int(matched["PRICE_CONTRACT"]).astype("float64")
    fc_ym = _carry_ym_int(matched["FORWARD_CONTRACT"]).astype("float64")
    ...
    raw_fc = (pd.Series(values, index=pd.to_datetime(matched["DATETIME"]))
    ...
    meta = {
        ...
        "days_strict": strict_days,
        "days_same_front_fb": same_front_fb_days,
        "days_any_front_fb": any_front_fb_days,
        "days_sign_mismatch_used": days_sign_mismatch_used,
        "flipped_columns": int('flipped' in locals() and flipped),
        "coverage": float(len(raw_fc)) / float(days_total) if days_total else 0.0,
    }
    return raw_fc, meta

def _carry_trimmed_mean_abs(s: pd.Series, q: float = None) -> float:
    if q is None: q = CFG.CARRY_TRIM_Q
...
def _carry_cap_aware_scale(smooth: pd.Series, target: float = None, cap: float = None,
                           q: float = None, symbol: Optional[str] = None) -> float:
    if target is None: target = CFG.CARRY_TARGET_ABS
    if cap    is None: cap    = CFG.CARRY_CAP
    if q      is None: q      = CFG.CARRY_TRIM_Q
    if smooth is None or smooth.empty or target <= 0: return 1.0
    nobs = int(smooth.notna().sum())
    if nobs < CFG.CARRY_MIN_OBS_HARD:
        return 1.0
    base_ma = _carry_trimmed_mean_abs(smooth, q=q)
    if nobs < CFG.CARRY_MIN_OBS_FOR_K and base_ma < 1e-6:
        return 1.0
    def f(k: float) -> float:
        return _carry_trimmed_mean_abs((smooth * k).clip(-cap, cap), q)
    if f(0.0) >= target: return 0.0
    lo, hi, val_hi, n_guard = 0.0, 1.0, f(1.0), 0
    while (val_hi < target) and (hi < 1e9) and (n_guard < 60):
        hi *= 2.0; val_hi = f(hi); n_guard += 1
    if val_hi < target:
        return min(1.0, _carry_cap_for_symbol(symbol))
    for _ in range(50):
        mid = 0.5 * (lo + hi)
        if f(mid) < target: lo = mid
        else:               hi = mid
    return min(0.5 * (lo + hi), _carry_cap_for_symbol(symbol))

def build_carry_series_carver(raw_forecast: pd.Series, symbol: Optional[str] = None) -> pd.DataFrame:
    if raw_forecast is None or len(raw_forecast) == 0:
        return pd.DataFrame(columns=["Date"] + [f"Carry{s}" for s in CFG.KEEP_CARRY])
    sym = symbol or ""
    is_com_heur = _carry_looks_like_commodity_symbol(sym)
    k_cap = _carry_cap_for_symbol(sym)
    if sym in SIGN_MAP:
        sign_mode = "explicit"
    elif sym in NON_COMMODITY_OVERRIDES:
        sign_mode = "none (override)"
    elif CFG.CARRY_APPLY_COMMODITY_SIGN and is_com_heur:
        sign_mode = "heuristic commodity flip"
    else:
        sign_mode = "none"
    log.info(f"{sym} classification: commodity_heur={is_com_heur} | k_cap={k_cap:g} | sign={sign_mode}")
    rf = _carry_maybe_flip_sign(sym, raw_forecast)
    out_idx = pd.DatetimeIndex(sorted(rf.index.unique()))
    out = pd.DataFrame(index=out_idx)
    for span in CFG.KEEP_CARRY:
        smooth = rf.reindex(out.index).ewm(span=int(span), adjust=False).mean()
        k = _carry_cap_aware_scale(smooth, target=CFG.CARRY_TARGET_ABS, cap=CFG.CARRY_CAP,
                                   q=CFG.CARRY_TRIM_Q, symbol=sym)
        series = (smooth * k).clip(-CFG.CARRY_CAP, CFG.CARRY_CAP)
        post_mean_abs = _carry_trimmed_mean_abs(series, q=CFG.CARRY_TRIM_Q)
        clip_rate = float((series.abs() >= CFG.CARRY_CAP - 1e-12).mean())
        nobs = int(smooth.notna().sum())
        reason = " (short history)" if nobs < CFG.CARRY_MIN_OBS_HARD and k == 1.0 else ""
        log.info(f"{sym} Span {span}: k={k:.4g}{reason}, post-cap trimmed|.|={post_mean_abs:.3f}, clip={clip_rate:.1%}")
...
def build_carry_signals_inline(chosen_map: Dict[str, str], force: Optional[bool] = None) -> Dict[str, int]:
    rolls_dir  = Path(CFG.FUT_ROLL_DIR)
    prices_dir = Path(CFG.FUT_PRICE_DIR)
    out_dir    = Path(CFG.CARRY_SIGNALS_DIR)
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "_debug").mkdir(exist_ok=True)
    if force is None:
        force = CFG.FORCE_REBUILD_CARRY
    symbols = sorted(set(chosen_map.values()))
    success = skipped = total_rows = 0
    def _is_fresh(sym: str, out_csv: Path) -> bool:
        if not out_csv.exists():
            return False
        try:
            mt_out = out_csv.stat().st_mtime
            src_mtime = max(
                (prices_dir / f"{sym}.csv").stat().st_mtime if (prices_dir / f"{sym}.csv").exists() else 0,
                (rolls_dir  / f"{sym}.csv").stat().st_mtime if (rolls_dir  / f"{sym}.csv").exists() else 0
            )
            return mt_out > src_mtime
        except Exception:
            return False
    for sym in symbols:
        ppath = _carry_price_path_for_symbol(sym, prices_dir)
        rpath = rolls_dir / f"{sym}.csv"
        if not ppath or not rpath.exists():
            log.warning(f"[CARRY] {sym}: missing {'price' if not ppath else 'roll'} CSV — skipping.")
            continue
        out_csv = out_dir / f"{sym}_carry_signals.csv"
        if (not force) and _is_fresh(sym, out_csv):
            skipped += 1
            continue
        try:
            roll_daily = load_roll_calendar(rpath)
            price_df   = load_price_data(ppath)
            raw_fc, meta = compute_raw_forecast(roll_daily, price_df, symbol=sym)
            if raw_fc.empty or len(raw_fc) < CFG.CARRY_MIN_ROWS:
                log.warning(
                    f"[CARRY] {sym}: no signals (len={len(raw_fc)}). "
                    f"strict={meta.get('days_strict',0)}, same-front-fb={meta.get('days_same_front_fb',0)}, "
                    f"any-front-fb={meta.get('days_any_front_fb',0)}, flipped={meta.get('flipped_columns',0)}, "
                    f"coverage={meta.get('coverage',0):.1%}"
                )
                continue
            final_df = build_carry_series_carver(raw_fc, symbol=sym)
            final_df["symbol"] = sym
            final_df.to_csv(out_csv, index=False)
            total_rows += len(final_df); success += 1
            dr = f"{final_df['Date'].min()} → {final_df['Date'].max()}"
            log.info(f"[CARRY] Wrote {out_csv.name}: {len(final_df):,} rows [{dr}]")
        except Exception as e:
            log.error(f"[CARRY] FAILED {sym}: {e}")
    log.info(f"[CARRY] Done. Successful: {success} | Skipped (fresh): {skipped} | Total rows: {total_rows} | Output: {out_dir}")
    return {"success": success, "skipped": skipped, "total_rows": total_rows, "n_syms": len(symbols)}

def winsorise_series(s: pd.Series, z: float = 5.0, window: int = 2520,
                     min_window: int = 504) -> Tuple[pd.Series, int, float]:
    if s.notna().sum() < 250:
        return s, 0, 0.0
...
def reshape_to_long(path: Path, root: str) -> pd.DataFrame:
    ...
    df = wide.copy()
    UNIT_FIX = {
        "Silver": .01, "GAS_": .01, "Corn_mini": .01, "Wheat_mini": .01, "Sugar": .01,
        "HG1": .01, "RB": .01
    }
    fac = next((UNIT_FIX[p] for p in UNIT_FIX if root.startswith(p.rstrip("_"))), 1)
    price_rows = (
        df[[date_col, pcon_col, price_col]].rename(columns={date_col:"Date", pcon_col:"Contract", price_col:"Close"})
        .assign(Close=lambda d: pd.to_numeric(d["Close"], errors="coerce") * fac, Kind="Price")
        .dropna(subset=["Close"])
    )
    ...
    if carry_col and ccon_col and (carry_col in df.columns) and (ccon_col in df.columns):
        carry_rows = (
            df[[date_col, ccon_col, carry_col]].rename(columns={date_col:"Date", ccon_col:"Contract", carry_col:"Close"})
            .assign(Close=lambda d: pd.to_numeric(d["Close"], errors="coerce") * fac, Kind="Carry")
            .dropna(subset=["Close"])
        )
    ...
    if fwd_col and fcon_col and (fwd_col in df.columns) and (fcon_col in df.columns):
        fwd_rows = (
            df[[date_col, fcon_col, fwd_col]].rename(columns={date_col:"Date", fcon_col:"Contract", fwd_col:"Close"})
            .assign(Close=lambda d: pd.to_numeric(d["Close"], errors="coerce") * fac, Kind="Forward")
            .dropna(subset=["Close"])
        )
    long = (pd.concat([price_rows, carry_rows, fwd_rows], ignore_index=True)
            .assign(Date=lambda d: _to_utc_index(pd.to_datetime(d["Date"]).dt.normalize()))
...
def build_xlarge_from_csv() -> pd.DataFrame:
    if not CFG.FUT_PRICE_DIR.exists():
        raise FileNotFoundError(f"Missing {CFG.FUT_PRICE_DIR}")
    if not CFG.FUT_ROLL_DIR.exists():
        raise FileNotFoundError(f"Missing {CFG.FUT_ROLL_DIR}")
    if not CFG.FUT_CFG_DIR.exists():
        raise FileNotFoundError(f"Missing {CFG.FUT_CFG_DIR}")
    inst = pd.read_csv(CFG.FUT_CFG_DIR / "instrumentconfig.csv").set_index("Instrument")
    cost = pd.read_csv(CFG.FUT_CFG_DIR / "spreadcosts.csv").set_index("Instrument")
    ...
    SKIP = {"HANGTECH","IBEX"} \
           | {p.stem for p in CFG.FUT_PRICE_DIR.glob("*_micro.csv")} \
           | {p.stem for p in CFG.FUT_PRICE_DIR.glob("*_mini.csv")}
    series = {}
    for csv_path in sorted(CFG.FUT_PRICE_DIR.glob("*.csv")):
        root = csv_path.stem
        if root in SKIP:
            continue
        long = reshape_to_long(csv_path, root)
        roll_file = CFG.FUT_ROLL_DIR / f"{root}.csv"
        ...
        if has_carry:
            carry_df = (long.query("Kind == 'Carry'")
                        .pivot_table(index="Date", columns="Contract", values="Close", aggfunc="last"))
        ...
        rf = pd.Series(0.0, index=total_ret.index)
        excess = (total_ret - daily_cost - rf.reindex(total_ret.index).fillna(0.0)).astype("float64")
        ...
        CFG.FUT_OUT_DIR.mkdir(parents=True, exist_ok=True)
        excess.to_csv(CFG.FUT_OUT_DIR / f"{root}_excess_net_ret.csv", float_format="%.8f")
        series[root] = excess
    if not series:
...
def load_sg() -> Tuple[pd.DatetimeIndex, pd.Series]:
    cal = pd.read_pickle(CFG.SG_CALENDAR_PATH)
    y = pd.read_pickle(CFG.SG_TARGET_PATH)
    cal = _to_utc_index(cal)
    cal = cal[~cal.duplicated(keep="last")].sort_values()
    y.index = _to_utc_index(y.index)
    y = y.sort_index().reindex(cal)
    log.info(f"SG Trend realized vol: {ann(y):.1%}")
    return cal, y

def load_xlarge() -> pd.DataFrame:
    if CFG.FORCE_REBUILD_XLARGE or (not Path(CFG.XLARGE_PATH).exists()):
        print("Assembling X_large_universe.parquet from CSVs ...")
    ...
    print("ℹSanity: per-contract ann vol (from parquet)\n", ann_vol.to_string())
    return X

PAPER_27 = [
    "AD","BP","CD","EC","JY","SF",
    "TU","FV","TY","US","G","RX","H",
    "ES","NQ","YM","Z","VG","AP","NKD",
    "GC","SI","HG","CL","HO","NG","XB"
]

PB_TO_RB_MAPPING = {
    "AD": "AUD", "BP": "GBP", "CD": "CAD", "EC": "EUR", "JY": "JPY", "SF": "CHF",
    "TU": "US2Y", "FV": "US5Y", "TY": "US10Y", "US": "US", "G": "G", "RX": "BUND", "H": "BOBL",
    "ES": "S&P500", "NQ": "NASDAQ", "YM": "DOW", "Z": "FTSE", "VG": "EUROSTOXX", "AP": "AP", "NKD": "TOPIX",
    "GC": "GOLD", "SI": "SILVER", "HG": "COPPER", "CL": "WTI_CRUDE", "HO": "GASOIL", "NG": "NG", "XB": "XB"
}
...
ALIAS = {
    ...
    "HO": ["GASOIL","HEATOIL","HEATOIL-ICE","HO"],
    "NG": ["GAS_US","GAS-LAST","GAS-PEN","NATGAS","NG"],
    "XB": ["GASOILINE","GASOILINE_ICE","RB","XB"],
}

ASSET_CLASS_MAP = {
    "AUD":"FX","GBP":"FX","CAD":"FX","EUR":"FX","JPY":"FX","CHF":"FX",
    "US2Y":"Rates","US5Y":"Rates","US10Y":"Rates","US":"Rates","G":"Rates","BUND":"Rates","BOBL":"Rates",
    "S&P500":"Equity","NASDAQ":"Equity","DOW":"Equity","FTSE":"Equity","EUROSTOXX":"Equity","AP":"Equity","TOPIX":"Equity",
    "GOLD":"Metals","SILVER":"Metals","COPPER":"Metals",
    "WTI_CRUDE":"Energy","GASOIL":"Energy","NG":"Energy","XB":"Energy"
}
...
def pick_alias_columns(X_all: pd.DataFrame) -> Tuple[Dict[str,str], List[str], List[str]]:
    ...
    for code in PAPER_27:
        rb_name = PB_TO_RB_MAPPING.get(code, code)
...
def load_carry_signals(carry_dir: Path,
                       chosen_map: Dict[str, str],
                       keep_spans: Tuple[int, ...],
                       cal: pd.DatetimeIndex) -> pd.DataFrame:
    data: Dict[Tuple[str, int], pd.Series] = {}
    for code, sym in chosen_map.items():
        path = carry_dir / f"{sym}_carry_signals.csv"
        if not path.exists():
            continue
        try:
            df = pd.read_csv(path)
...
def _risk_scale_to_pre_target(strat_pre: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in strat_pre.columns:
        out[col] = rescale_series_to_target(
            strat_pre[col],
            target=CFG.PRE_SCALE_TARGET,
            span=CFG.EWMA_SPAN_VOL,
            cap=CFG.PRE_SCALE_CAP,
            floor=CFG.PRE_SCALE_FLOOR,
            keep_na=True
        )
    return out

def build_strategy_families(returns_panel: pd.DataFrame,
                            trend_lbs: np.ndarray,
                            breakout_lbs: Tuple[int, ...],
                            skewabs_lbs: Tuple[int, ...],
                            accel_lbs: Tuple[int, ...],
    ...
    for lb in trend_lbs:
        ...
        z  = (mu / sd).clip(-CFG.Z_CAP, CFG.Z_CAP)
        z  = z.ewm(span=max(2, lb // 4), adjust=False).mean()
        w = z.shift(1) / sigma40_ann
        strat_pre = w * r
        strat_scaled = _risk_scale_to_pre_target(strat_pre)
        strat_scaled.columns = pd.MultiIndex.from_product(
            [["TREND"], list(returns_panel.columns), [lb]],
            names=["family","contract","lookback"]
        )
    ...
    if breakout_lbs:
        ...
        for lb in breakout_lbs:
            ...
            sig = pd.DataFrame(
                np.where(price_idx.gt(roll_max), 1.0,
                         np.where(price_idx.lt(roll_min), -1.0, 0.0)),
            ...
            strat_scaled = _risk_scale_to_pre_target(strat_pre)
            strat_scaled.columns = pd.MultiIndex.from_product(
                [["BREAKOUT"], list(returns_panel.columns), [lb]],
                names=["family","contract","lookback"]
            )
            pieces.append(strat_scaled)
    if skewabs_lbs:
        for lb in skewabs_lbs:
            ...
            strat_scaled = _risk_scale_to_pre_target(strat_pre)
            strat_scaled.columns = pd.MultiIndex.from_product(
                [["SKEWABS"], list(returns_panel.columns), [lb]],
                names=["family","contract","lookback"]
            )
            pieces.append(strat_scaled)
    if accel_lbs:
        for lb in accel_lbs:
            ...
            z = ((short - long) / sd_s).clip(-CFG.Z_CAP, CFG.Z_CAP)
            z = z.ewm(span=max(2, lb // 4), adjust=False).mean()
            w = z.shift(1) / sigma40_ann
            strat_pre = w * r
            strat_scaled = _risk_scale_to_pre_target(strat_pre)
            strat_scaled.columns = pd.MultiIndex.from_product(
                [["ACCEL"], list(returns_panel.columns), [lb]],
                names=["family","contract","lookback"]
            )
            pieces.append(strat_scaled)
    if (carry_panel is not None) and (not carry_panel.empty):
        ...
        if carry_pre:
            ...
            print(f"   CARRY family added: {len(set(carry_panel.columns.get_level_values('contract')))} contracts,"
                  f" spans {sorted(set(carry_panel.columns.get_level_values('lookback')))}")
    Rfam = pd.concat(pieces, axis=1).sort_index(axis=1)
    sample_cols = [c for c in Rfam.columns if c[0]=="TREND" and c[2] in (60,120,180)]
    if sample_cols:
        v = [ann(Rfam[c].dropna().tail(252)) for c in sample_cols]
        print(f"   recent TREND vols (60/120/180): {np.nanmean(v):.1%} (target ≈ 20%)")
    return Rfam

def cv_lambda_on_corr(R: pd.DataFrame, y: pd.Series,
                      lambdas: np.ndarray, n_folds: int = 5,
                      end_date: Optional[pd.Timestamp] = None,
                      purge_gap_days: int = CFG.PURGE_GAP_DAYS) -> float:
    ...
    if all((not np.isfinite(v)) or (v == -np.inf) for v in scores.values()):
        best = float(lambdas[0])
        log.warning("CV produced no finite scores; falling back to smallest lambda.")
    else:
        best = max(scores, key=lambda k: scores[k])
    log.info(f"CV best lambda={best:.6f} (CV corr={scores[best]:.3f})")
    return float(best)

def fit_quarterly_betas(R: pd.DataFrame, y: pd.Series,
                        lam: float, fix_after: Optional[pd.Timestamp]) -> pd.DataFrame:
    q_start = R.index.min()
    q_end   = R.index.max()
    ...
    B = B.reindex(daily).ffill()
    B = B.reindex(R.index).ffill()
    B = B.ewm(span=CFG.WEIGHT_EMA_SPAN, adjust=False).mean()
    first = B.index[0]
    B.loc[B.index < first + pd.Timedelta(days=CFG.STARTUP_DELAY_DAYS)] = 0.0
    return B

def apply_soft_start(W: pd.DataFrame, n_days: int) -> pd.DataFrame:
    if n_days <= 0 or W.empty:
        return W
    idx = W.index
    if len(idx) <= 1:
        return W
    end_ix = min(n_days, len(idx))
    ramp = np.linspace(1/(end_ix), 1.0, end_ix)
    W2 = W.copy()
    W2.iloc[:end_ix, :] = (W2.iloc[:end_ix, :].T * ramp).T
    return W2
...
def generate_replication(R: pd.DataFrame, beta_daily: pd.DataFrame,
                         final_target: float,
                         cost_bps: float,
                         overlay_cost_bps: float) -> Tuple[pd.Series, pd.Series, pd.Series, pd.Series]:
    beta_daily = beta_daily.reindex(columns=R.columns, fill_value=0.0)
    raw = (R.fillna(0.0) * beta_daily).sum(axis=1)
    sigma = raw.ewm(
        span=CFG.EWMA_SPAN_VOL,
        adjust=False,
        min_periods=CFG.STARTUP_DELAY_DAYS
    ).std().shift(1) * np.sqrt(CFG.ANN_DAYS)
    sigma = sigma.replace([0, np.inf], np.nan).bfill()
    scale = (final_target / sigma).clip(lower=CFG.FINAL_SCALE_FLOOR, upper=CFG.FINAL_SCALE_CAP)
    scale = scale.fillna(method="bfill").fillna(method="ffill")
...
def performance_table(rep: pd.Series, y: pd.Series, start="2001-01-02") -> pd.DataFrame:
    ...
    out = pd.DataFrame(index=["Replication","Benchmark"], columns=[
        "Annualized Return","Annualized Volatility","Sharpe Ratio","Max Drawdown","Correlation","Tracking Error"
    ])
    def ann_ret(x): return (1+ x).prod()**(252/len(x)) - 1 if len(x)>0 else np.nan
    def dd(x):
        c = (1+x).cumprod(); peak = c.cummax()
        return (c/peak - 1).min()
...
def annotate_crisis_periods(ax):
    ...
    for start, end, label in CRISIS_PERIODS:
        ...
        ax.text(sdt + (edt - sdt)/2, ax.get_ylim()[1]*0.94, label,
...
def make_rev26h_style_figures_process(rep: pd.Series,
                                      y: pd.Series,
                                      beta_daily: pd.DataFrame,
                                      weight_by_lb_daily: pd.DataFrame,
                                      turnover_portfolio: pd.Series):
    try:
        rep = rep.dropna()
        ...
        y_c   = 100 * (1 + y).cumprod()
        ax.plot(_to_naive(rep_c.index), rep_c.values, label="Process-Based Replication", color=COLOR_PALETTE['model'], lw=1.8)
        ax.plot(_to_naive(y_c.index),   y_c.values,     label="SG Trend Index", color=COLOR_PALETTE['benchmark'], lw=1.5)
        ax.set_yscale("log"); ax.grid(True, alpha=0.3)
        ax.set_title(f"Cumulative Growth of $100 (Process-Based)")
        ...
        plt.tight_layout(); plt.savefig(CFG.VIZ_DIR / "A_cumulative_equity_curves_process_v6.png", bbox_inches="tight")
        ...
        if len(rep_b) >= w + 20:
            ...
            plt.tight_layout(); plt.savefig(CFG.VIZ_DIR / "B_rolling_statistics_process_v6.png", bbox_inches="tight")
            if SHOW_FIGS: plt.show(); plt.close(fig)
            else: plt.close(fig)
        ...
        plt.tight_layout(); plt.savefig(CFG.VIZ_DIR / "E_lookback_weight_share_process_v6.png", bbox_inches="tight")
        if SHOW_FIGS: plt.show(); plt.close(fig)
        else: plt.close(fig)
        print(f"Base figures A/B/E saved to: {CFG.VIZ_DIR}")
    except Exception as e:
        print(f"REV-26h visuals failed: {e}")

def make_rev26h_extras(rep: pd.Series,
                       y: pd.Series,
                       scale: pd.Series,
                       beta_daily: pd.DataFrame,
                       R_eff: pd.DataFrame):
    ...
    try:
        ...
        plt.tight_layout(); plt.savefig(CFG.VIZ_DIR / "D_overlay_scale_process_v6.png", bbox_inches="tight")
        if SHOW_FIGS: plt.show(); plt.close(fig)
        else: plt.close(fig)
    except Exception as e:
    ...
    try:
        ...
        plt.tight_layout(); plt.savefig(CFG.VIZ_DIR / "F_residuals_hist_process_v6.png", bbox_inches="tight")
        if SHOW_FIGS: plt.show(); plt.close(fig)
        else: plt.close(fig)
        fig, ax = plt.subplots(figsize=(14,4))
        ...
        plt.tight_layout(); plt.savefig(CFG.VIZ_DIR / "F_residuals_cum_process_v6.png", bbox_inches="tight")
        if SHOW_FIGS: plt.show(); plt.close(fig)
        else: plt.close(fig)
    except Exception as e:
...
def export_turnover_by_lookback(W_eff_adj: pd.DataFrame):
    ...
    by_lb_daily.to_csv(CFG.VIZ_DIR / "turnover_by_lookback_daily_process_v6.csv", float_format="%.8f")
    mean_d = by_lb_daily.mean(axis=0)
    std_d  = by_lb_daily.std(axis=0)
    summary = pd.DataFrame({
        "lookback": mean_d.index.astype(int),
        "mean_daily": mean_d.values,
        "std_daily": std_d.values,
        "annual": (mean_d * 252).values
    }).sort_values("lookback")
    summary.to_csv(CFG.VIZ_DIR / "turnover_by_lookback_process_v6.csv",
                   index=False, float_format="%.8f")
    pretty = summary.copy()
    pretty["mean_daily"] = 100 * pretty["mean_daily"]
    pretty["annual"]     = 100 * pretty["annual"]
    print("\nTurnover by lookback — annualized % of notional")
    print(pretty.to_string(index=False,
                           formatters={"mean_daily": "{:.4f}%".format,
                                       "std_daily": "{:.6f}".format,
                                       "annual": "{:.2f}%".format}))
    fig, ax = plt.subplots(figsize=(14, 8))
    ...
    plt.tight_layout()
    plt.savefig(CFG.VIZ_DIR / "C_turnover_by_lookback_process_v6.png", bbox_inches="tight")
    if SHOW_FIGS:
        plt.show()
        plt.close(fig)
    else:
...
def export_implementation_quality(rep: pd.Series, y: pd.Series,
                                  scale: pd.Series,
                                  turn_w: pd.Series, turn_s: pd.Series,
                                  W_eff_adj: Optional[pd.DataFrame]):
    idx = rep.index.intersection(y.index)
    r, b = rep.loc[idx], y.loc[idx]
    res = r - b
    ...
    out = pd.DataFrame([{
        ...
        "days": int(len(idx)),
        "corr": corr, "R2": r2, "TE": te, "IR": ir,
        "rep_ann_return": ann_r, "bench_ann_return": ann_b,
        "mean_overlay": float(scale.mean()),
        "pct_days_at_cap": float(cap_frac),
        "pct_days_at_floor": float(floor_frac),
        "mean_live_L1": float(live_l1) if pd.notna(live_l1) else np.nan,
        "avg_replication_cost_bps_per_day": float(cost_w_bps),
        "avg_overlay_cost_bps_per_day": float(cost_s_bps),
    }])
...
def export_oos_performance(rep: pd.Series, y: pd.Series):
    def stats(r, b):
        ...
        d = {
            ...
            "CAGR": (1.0 + r).prod()**(252/max(len(r),1)) - 1.0 if len(r) else np.nan
        }
        return d
    oos = CFG.OOS_START
    idx = rep.index.intersection(y.index)
...
def make_calendar_year_bars(rep: pd.Series, y: pd.Series):
    ...
    tbl.to_csv(CFG.VIZ_DIR / "calendar_year_table_process_v6.csv", float_format="%.8f")
    fig, ax = plt.subplots(figsize=(14, 8))
    ...
    plt.tight_layout(); plt.savefig(CFG.VIZ_DIR / "Y_calendar_year_bars_process_v6.png", bbox_inches="tight")
    if SHOW_FIGS: plt.show(); plt.close(fig)
    else: plt.close(fig)

def make_monthly_heatmap(rep: pd.Series,
                         y: pd.Series,
                         annotate: bool = True,
                         fmt: str = "{:.1f}",
                         diverging: bool = True):
    ...
    df.to_csv(CFG.VIZ_DIR / "monthly_returns_process_v6.csv", float_format="%.8f")
    mat = df["model"].copy()
    ...
    plt.tight_layout(); plt.savefig(CFG.VIZ_DIR / "I_monthly_return_heatmap_process_v6.png", bbox_inches="tight")
    if SHOW_FIGS: plt.show(); plt.close(fig)
    else: plt.close(fig)
...
def make_monthly_scatter_regime(rep: pd.Series, y: pd.Series):
    ...
    tmp.to_csv(CFG.VIZ_DIR / "monthly_scatter_data_process_v6.csv", float_format="%.8f")
    colors = {
        "Strong Bear":"#b2182b", "Bear":"#ef8a62", "Weak Bear":"#fddbc7",
        "Neutral":"#cccccc", "Weak Bull":"#d1e5f0", "Bull":"#67a9cf", "Strong Bull":"#2166ac"
    }
    fig, ax = plt.subplots(figsize=(14, 8))
    ...
    plt.tight_layout(); plt.savefig(CFG.VIZ_DIR / "C_monthly_scatter_regime_process_v6.png", bbox_inches="tight")
    if SHOW_FIGS: plt.show(); plt.close(fig)
    else: plt.close(fig)
...
def make_drawdown_analysis(rep: pd.Series, y: pd.Series):
    ...
    diff = dd_r - dd_b
    dd_tbl = pd.DataFrame({
        "stat": ["min_model_dd","min_bench_dd","min_diff_dd"],
        "value": [dd_r.min(), dd_b.min(), diff.min()]
    })
    dd_tbl.to_csv(CFG.VIZ_DIR / "drawdown_stats_process_v6.csv", index=False, float_format="%.8f")
    ...
    for ax in (ax1, ax2):
        ...
        ax.axvline(_to_naive(CFG.OOS_START), color="black", ls="--", lw=2, alpha=0.7)
        annotate_crisis_periods(ax)
    plt.tight_layout(); plt.savefig(CFG.VIZ_DIR / "H_drawdown_analysis_process_v6.png", bbox_inches="tight")
    if SHOW_FIGS: plt.show(); plt.close(fig)
    else: plt.close(fig)

def export_signal_bars_and_csv(R_eff: pd.DataFrame, W_eff_adj: pd.DataFrame,
                               scale: pd.Series, y_eff: pd.Series):
    ...
    fam_m.to_csv(CFG.VIZ_DIR / "pb_signal_family_contrib_monthly_process_v6.csv", float_format="%.8f")
    if len(fam_m) > 0:
        ...
        ax.axvline(_to_naive(CFG.OOS_START), color="black", ls="--", lw=2, alpha=0.7)
        _save_only(fig, CFG.VIZ_DIR / "PB_signal_family_contrib_monthly_process_v6.png")
        fam_y = fam_daily.resample("Y").sum()
        fam_y.index = fam_y.index.year
        fam_y.to_csv(CFG.VIZ_DIR / "pb_signal_family_contrib_calendar_process_v6.csv", float_format="%.8f")
        fig, ax = plt.subplots(figsize=(14, 8))
        ...
        ax.grid(True, axis='y', alpha=0.3); ax.legend(ncol=5, fontsize=8, bbox_to_anchor=(1.0, 1.02), loc="upper right")
        plt.tight_layout(); plt.savefig(CFG.VIZ_DIR / "PB_signal_family_contrib_calendar_bars_process_v6.png", bbox_inches="tight")
        if SHOW_FIGS: plt.show(); plt.close(fig)
        else: plt.close(fig)
    lb_daily = contrib_daily.groupby(axis=1, level='lookback').sum()
    lb_m = lb_daily.resample("M").sum()
    lb_m.to_csv(CFG.VIZ_DIR / "pb_signal_lookback_contrib_monthly_process_v6.csv", float_format="%.8f")
    if len(lb_m) > 0:
        ...
        ax.axvline(_to_naive(CFG.OOS_START), color="black", ls="--", lw=2, alpha=0.7)
        _save_only(fig, CFG.VIZ_DIR / "PB_signal_lookback_contrib_monthly_process_v6.png")
        ...
        ax.grid(True, axis='y', alpha=0.3); ax.legend(ncol=6, fontsize=8, bbox_to_anchor=(1.0, 1.02), loc="upper right")
        plt.tight_layout(); plt.savefig(CFG.VIZ_DIR / "PB_signal_lookback_contrib_calendar_bars_process_v6.png", bbox_inches="tight")
        if SHOW_FIGS: plt.show(); plt.close(fig)
        else: plt.close(fig)
    ctrt_daily = contrib_daily.groupby(axis=1, level='contract').sum()
    ctrt_daily.to_csv(CFG.VIZ_DIR / "pb_signal_contract_contrib_daily_process_v6.csv", float_format="%.8f")
    ...
    if top:
        ...
        ax.grid(True, axis='y', alpha=0.3)
        plt.tight_layout(); plt.savefig(CFG.VIZ_DIR / "PB_signal_contract_top10_contrib_bars_process_v6.png", bbox_inches="tight")
        if SHOW_FIGS: plt.show(); plt.close(fig)
        else: plt.close(fig)
        ...
        ax.set_title("Top‑10 Contracts — Rolling 3m Contribution (%)")
        ax.yaxis.set_major_formatter(PercentFormatter(100.0))
        ax.grid(True, alpha=0.3); ax.legend(ncol=5)
        plt.tight_layout(); plt.savefig(CFG.VIZ_DIR / "PB_signal_contract_top10_contrib_process_v6.png", bbox_inches="tight")
        if SHOW_FIGS: plt.show(); plt.close(fig)
        else: plt.close(fig)
    ...
    fam_te.to_frame("te_contribution").to_csv(CFG.VIZ_DIR / "pb_signal_family_TE_contrib_process_v6.csv", float_format="%.8f")
    ...
    ax.grid(True, axis='y', alpha=0.3)
    plt.tight_layout(); plt.savefig(CFG.VIZ_DIR / "PB_signal_family_TE_contrib_process_v6.png", bbox_inches="tight")
    if SHOW_FIGS: plt.show(); plt.close(fig)
    else: plt.close(fig)
...
def export_library_coverage_heatmap(R_eff: pd.DataFrame):
    ...
    meta = pd.DataFrame({
        "family":   R_eff.columns.get_level_values("family"),
        "contract": R_eff.columns.get_level_values("contract"),
        "lookback": R_eff.columns.get_level_values("lookback").astype(int),
        "live_frac": live.values
    })
    agg = (meta.groupby(["family", "lookback"])
                .agg(n_contracts=("contract", "nunique"),
                     avg_live=("live_frac", "mean"))
                .reset_index())
    agg["effective_series"] = agg["n_contracts"] * agg["avg_live"]
    agg.to_csv(CFG.VIZ_DIR / "pb_library_summary_process_v6.csv", index=False, float_format="%.6f")
    ...
    plt.tight_layout(); plt.savefig(CFG.VIZ_DIR / "PB_library_coverage_heatmap_process_v6.png", bbox_inches="tight")
    if SHOW_FIGS: plt.show(); plt.close(fig)
    else: plt.close(fig)
...
def export_cv_lambda_path(R: pd.DataFrame, y: pd.Series):
    ...
    df.to_csv(CFG.VIZ_DIR / "pb_cv_lambda_summary_process_v6.csv", index=False, float_format="%.6f")
    fig, ax = plt.subplots(figsize=(14, 6))
    ...
    _save_only(fig, CFG.VIZ_DIR / "PB_cv_lambda_path_process_v6.png")
...
def export_cost_decomposition(turnover_w: pd.Series, turnover_s: pd.Series):
    ...
    df.to_csv(CFG.VIZ_DIR / "pb_cost_decomposition_process_v6.csv", float_format="%.6f")
    fig, ax = plt.subplots(figsize=(14,8))
    ax.bar(df.index, df["replication_bps_per_month"], label="Replication trading (bps/mo)")
    ax.bar(df.index, df["overlay_bps_per_month"], bottom=df["replication_bps_per_month"],
           alpha=0.35, label="Overlay rebalancing (bps/mo)")
    ax.set_title("Cost Decomposition (monthly bps, one-way)")
    ax.grid(True, axis='y', alpha=0.3); ax.legend()
    plt.tight_layout(); plt.savefig(CFG.VIZ_DIR / "PB_cost_decomposition_process_v6.png", bbox_inches="tight")
    if SHOW_FIGS: plt.show(); plt.close(fig)
    else: plt.close(fig)
...
def export_timeavg_lookback_weights_csv(W_eff_adj: pd.DataFrame):
    ...
    out.to_csv(CFG.VIZ_DIR / "pb_timeavg_lookback_weights_process_v6.csv", index=False, float_format="%.8f")
...
def plot_frozen_vs_live_oos(rep_live: pd.Series, rep_frozen_scaled_net: pd.Series, bench: pd.Series):
    ...
    plt.tight_layout(); plt.savefig(CFG.VIZ_DIR / "PB_frozen_vs_live_oos_process_v6.png", bbox_inches="tight")
    if SHOW_FIGS: plt.show(); plt.close(fig)
    else: plt.close(fig)

def build_asset_class_rolling_contrib(R_eff: pd.DataFrame, W_eff_adj: pd.DataFrame, scale: pd.Series, window: int = 63) -> pd.DataFrame:
    contrib_daily = R_eff.mul(W_eff_adj, fill_value=0.0)
    contrib_daily = (contrib_daily.T * scale.reindex(contrib_daily.index)).T
    ctrt_daily = contrib_daily.groupby(axis=1, level='contract').sum()
    if ctrt_daily.empty:
        return pd.DataFrame(index=contrib_daily.index)
    class_map = pd.Series({c: ASSET_CLASS_MAP.get(c, "Other") for c in ctrt_daily.columns})
    class_daily = ctrt_daily.groupby(class_map, axis=1).sum()
    rolling = class_daily.rolling(window).sum()
    return rolling
...
def main():
    print("\n==================================================================")
    print("PROCESS-BASED SG TREND")
    print("==================================================================\n")
    CFG.OUT_DIR.mkdir(parents=True, exist_ok=True)
    CFG.VIZ_DIR.mkdir(parents=True, exist_ok=True)
    print("Loading SG calendar / target ...")
    cal, y = load_sg()
    print(f"SG Trend realized vol: {ann(y):.1%}\n")
    X_all = load_xlarge()
    ...
    if CFG.USE_CARRY and CFG.KEEP_CARRY:
        if CFG.BUILD_CARRY_INLINE:
            print("\Building  carry signals (inline) ...")
            build_summary = build_carry_signals_inline(chosen_map=chosen, force=CFG.FORCE_REBUILD_CARRY)
            print(f"   Carry build summary: {build_summary}")
        print("Loading  carry signals ...")
        CFG.CARRY_SIGNALS_DIR.mkdir(parents=True, exist_ok=True)
        carry_panel = load_carry_signals(
            CFG.CARRY_SIGNALS_DIR,
            chosen_map=chosen,
            keep_spans=CFG.KEEP_CARRY,
            cal=cal
        )
    ...
    print("\n Building strategy library (multi-family) + per-strategy pre-scale (t-1) ...")
    R = build_strategy_families(
        returns_panel=X27,
        trend_lbs=CFG.LOOKBACKS,
        breakout_lbs=CFG.KEEP_BREAKOUT,
        skewabs_lbs=CFG.KEEP_SKEWABS,
        accel_lbs=CFG.KEEP_ACCEL,
        carry_panel=carry_panel
    )
    (CFG.OUT_DIR / "library").mkdir(parents=True, exist_ok=True)
    ...
    W_eff_adj = availability_adjust_weights(
        W_eff, R_eff, cap=CFG.MAX_SINGLE_WEIGHT, l1_target=CFG.L1_TARGET
    )
    ...
    if pd.notna(gate):
        ...
        if CFG.SOFT_START_DAYS > 0:
            W_eff_adj = apply_soft_start(W_eff_adj, CFG.SOFT_START_DAYS)
        W_eff_adj.to_parquet(CFG.OUT_DIR / "weights_daily_availability_adjusted_gated.parquet",
                             compression="snappy")
    else:
    ...
    weight_by_lb_avg = weight_by_lb_daily_eff.mean().sort_index()
    try:
        weight_by_lb_avg.index = weight_by_lb_avg.index.astype(int)
    except Exception:
        pass
    print("\n Generating replication + final overlay (t-1) ...")
    rep, scale, turnover_w, turnover_s = generate_replication(
        R_eff, W_eff_adj,
        final_target=CFG.FINAL_VOL_TARGET,
        cost_bps=CFG.REPL_TRADE_COST_BPS,
        overlay_cost_bps=CFG.OVERLAY_TRADE_COST_BPS
    )
    ...
    make_rev26h_style_figures_process(rep=rep,
                                      y=y_eff,
                                      beta_daily=W_eff_adj,
                                      weight_by_lb_daily=weight_by_lb_daily_eff,
                                      turnover_portfolio=turnover_w)
    ...
    ac_roll = build_asset_class_rolling_contrib(R_eff, W_eff_adj, scale, window=63)
    ac_roll.to_csv(CFG.VIZ_DIR / "pb_asset_class_rolling_contrib_daily_process_v6.csv", float_format="%.8f")
    plot_asset_class_rolling_contrib(ac_roll, CFG.VIZ_DIR / "PB_asset_class_rolling_contrib_process_v6.png")
    ...
    pd.DataFrame({
        "segment": ["IS","OOS"],
        "corr":    [is_corr, oos_corr],
        "TE":      [is_te, oos_te],
        "SR":      [is_sr, oos_sr],
        "vol":     [is_vol, oos_vol]
    }).to_csv(CFG.OUT_DIR / "oos_split_process_v6.csv", index=False, float_format="%.6f")
    ...
    frozen_sigma = rep_oos_frozen.ewm(
        span=CFG.EWMA_SPAN_VOL, adjust=False, min_periods=CFG.STARTUP_DELAY_DAYS
    ).std().shift(1) * np.sqrt(CFG.ANN_DAYS)
    frozen_sigma = frozen_sigma.replace([0, np.inf], np.nan).bfill()
    frozen_scale = (CFG.FINAL_VOL_TARGET / frozen_sigma).clip(
        lower=CFG.FINAL_SCALE_FLOOR, upper=CFG.FINAL_SCALE_CAP
    ...
    cfg_dump = {
        ...
        "FORCE_REBUILD_CARRY": CFG.FORCE_REBUILD_CARRY,
        "CARRY_TARGET_ABS": CFG.CARRY_TARGET_ABS,
        "CARRY_CAP": CFG.CARRY_CAP,
        "CARRY_TRIM_Q": CFG.CARRY_TRIM_Q,
        "CARRY_K_MAX": CFG.CARRY_K_MAX,
        "CARRY_K_MAX_EQ": CFG.CARRY_K_MAX_EQ,
        "CARRY_MIN_OBS_FOR_K": CFG.CARRY_MIN_OBS_FOR_K,
        "CARRY_MIN_OBS_HARD": CFG.CARRY_MIN_OBS_HARD,
        "CARRY_MIN_ROWS": CFG.CARRY_MIN_ROWS,
        "CARRY_SIGMA_FLOOR_PCT": CFG.CARRY_SIGMA_FLOOR_PCT,
        "CARRY_SIGMA_FLOOR_MIN": CFG.CARRY_SIGMA_FLOOR_MIN,
        "CARRY_VOL_SHORT": CFG.CARRY_VOL_SHORT,
        "CARRY_VOL_LONG": CFG.CARRY_VOL_LONG,
        "CARRY_W_SHORT": CFG.CARRY_W_SHORT,
        "CARRY_VOL_DENOM": CFG.CARRY_VOL_DENOM,
        "CARRY_FALLBACK_ALLOW_ANY_FRONT": CFG.CARRY_FALLBACK_ALLOW_ANY_FRONT,
        "CARRY_APPLY_COMMODITY_SIGN": CFG.CARRY_APPLY_COMMODITY_SIGN,
    }
